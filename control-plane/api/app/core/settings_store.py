...
SPECS: tuple[SettingSpec, ...] = (
    ...
    SettingSpec("claude_model", "Claude model", "Agents",
                relevant_when=("llm_provider_adapter", "claude"),
    ...
    SettingSpec("max_node_retries", "Node retries", "Agents", type="int", advanced=True,
    ...
    SettingSpec("dispatch_timeout_seconds", "Dispatch timeout (s)", "Remote execution",
    ...
    SettingSpec("reconciler_interval_seconds", "Reconciler interval (s)", "Remote execution",
                type="float", advanced=True,
                help="How often results are pulled for work running elsewhere."),
    SettingSpec("local_dispatch_duration_seconds", "Simulated job duration (s)",
                "Remote execution", type="float", advanced=True,
                relevant_when=("work_dispatch_adapter", "local"),
                help="How long the simulated job pretends to take."),
    ...
    SettingSpec("copilot_model", "Copilot model", "Implementation",
                relevant_when=("implementation_agent", "github-copilot"), advanced=True,
    ...
    # --- context graph ---
    SettingSpec("requirements_source_adapter", "Requirements from", "Intake", type="enum",
                options=("csv", "jira"),
                help="csv accepts pasted text or an upload; jira reads issues read-only."),
    SettingSpec("jira_base_url", "Jira site", "Intake", section="engagement",
                relevant_when=("requirements_source_adapter", "jira"),
                placeholder="https://acme.atlassian.net"),
    SettingSpec("jira_email", "Jira account email", "Intake", section="engagement",
                relevant_when=("requirements_source_adapter", "jira"),
                help="The account the API token belongs to."),
    SettingSpec("jira_api_token", "Jira API token", "Credentials",
                section="credential", kind="secret",
                help="Read from the environment. Needs Browse Projects on the issues you index."),
    SettingSpec("jira_query", "Default JQL", "Intake", section="engagement", advanced=True,
                relevant_when=("requirements_source_adapter", "jira"),
                placeholder="project = ACME AND type = Story AND status != Done",
                help="Used when a run names no issue key."),
